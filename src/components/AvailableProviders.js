...
const AvailableProviders = () => {
    const location = useLocation();
    const navigate = useNavigate();
    const { pincode, service, providers = [], bookingId, bookingDate, bookingTime } = location.state || {};
    ...
    const handleBooking = async (provider) => {
        // Create a reference to the existing booking
        const bookingRef = doc(db, 'bookings', bookingId);

        // Update the booking with the provider's name
        try {
            await updateDoc(bookingRef, {
                providerName: provider.name,
            });
            console.log("Provider name added to booking:", provider.name);
